...
namespace MAUISql.Data
{
    public class DatabaseContext : IAsyncDisposable
    {
        ...
        private SQLiteAsyncConnection Database => _connection;
        public DatabaseContext(string connectionString)
        {
            _connection = new SQLiteAsyncConnection(connectionString, SQLiteOpenFlags.Create | SQLiteOpenFlags.ReadWrite | SQLiteOpenFlags.SharedCache);
        }
        ...
        public async Task<TTable> FindAsync<TTable>(TTable item) where TTable : class, new() //method for key search
        {
            var keys = await GetAllAsync<TTable>();
            var foundItem = keys.FirstOrDefault(key => key.Equals(item));
            return foundItem;
        }
    }
}
